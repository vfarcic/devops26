...
That was just another useful command in `jx`. It opened GitHub repository in your browser.

![Figure 4-TODO: TODO:](images/ch04/github-go-demo-6.png)

Let's us quickly explore the files of the project, before we import it to GitHub.
...
That project is one extreme of the possible spectrum of projects we might want to import to Jenkins X. It only has the code of the application. There is no Dockerfile and there is no Helm chart. Heck, there is not even a script for building the binary or to run tests, and there is definitelly no Jenkinsfile that defines a continuous delivery pipelines for the application. There's only code, and (almost) nothing else.

Such a situation might not be your case. Maybe you do have scripts for running tests or building the code. Or maybe you do have Dockerfile and maybe you are already a heavy Kubernetes user and you do have a Helm chart. You might have other files as well. We'll discuss those situations later.
...
We can see that quite a few new files were added to the project. We got `Dockerfile` that will be used to build container images and `Jenkinsfile` that defines all the steps of our pipeline. Further on, we got `Makefile` that, among others, defines targets to build, test, and install the application. Further on, there is now the `charts` directory that contains files in Helm format. We'll use it to package, install, and upgrade our application. Then, there is `skaffold.yaml` that contains instructions how to build container images. Finally, we got `watch.sh` which we will not use. It allows us to compile the binary and create a container image every time we change a file in that project. It is a very useful script for local development.
...
Now that the project is in Jenkins X, we should see it as one of the activities and see the first build in action. You already know that we can limit the retrieval of Jenkins X activities to a specific project and that we can use `-w` to watch the progress.
...
So far, the end-result looks similar as the one we got when we created a quickstart. Jenkins X created the files it needs, it created a GitHub webook, it created a job in Jenkins, and it pushed changes to GitHub. As a result, we got our first build and, by the look of it, it was successful.
...
The problem is in the database or, to be more precise, in lack one. The application tried to connect to MongoDB and it couldn't find one.
...
I> Please note that we'll explore how to fix the issues that occured from importing a specific application into Jenkins X. Your problems will likely be different. Nevertheless, I believe that the exercises that follow will give you an insight into what to expect when importing projects as well as watch out for auto-generated Helm charts as being the most common culprit.
...
First of all, the application requires an environment variable called `DB`. The code is using it to obtain the address of the database. That brings us to the first thing missing in the chart generated by Jenkins X.
...
There's only one more thing missing. We should probably customize MongoDB chart to fit our use case. I won't go through all the values we could set. You can explore them yourself by executing `helm inspect values stable/mongodb` or vising the [project README](https://github.com/helm/charts/tree/master/stable/mongodb). Instead, we'll define only one, mostly as an execise how to define values for the dependencies.
...
We set only `replicaSet.enabled` to `true`. The important thing to note is that it is nested inside `go-demo-6-db`. That way, Helm will know that variable is not meant for our application (*go-demo-6*), but for the dependency called (aliased) `go-demo-6-db`.
...
The good news is that the database is indeed running. The bad news are that the application is still restarting. In my case, it already restarted five times and `0` containers are available.

Given that this time the problem is probably not related to the database, the logical course of action is to describe the Pod and see whether we can get a clue about the issue from the events.
...
The output, limited to the events, is as follows.
...
Events:
  ...
  ----     ------                 ----                   ----                                              -------
  Normal   Scheduled              2m46s                  default-scheduler                                 Successfully assigned jx-staging-go-demo-6-fdd8f6644-xx68f to gke-jx-rocks-default-pool-119fec1e-v7p7
  Normal   SuccessfulMountVolume  2m46s                  kubelet, gke-jx-rocks-default-pool-119fec1e-v7p7  MountVolume.SetUp succeeded for volume "default-token-cxn5p"
  Warning  Unhealthy              2m29s (x2 over 2m39s)  kubelet, gke-jx-rocks-default-pool-119fec1e-v7p7  Readiness probe failed: Get http://10.28.2.17:8080/: dial tcp 10.28.2.17:8080: getsockopt: connection refused
  Warning  BackOff                2m13s (x2 over 2m21s)  kubelet, gke-jx-rocks-default-pool-119fec1e-v7p7  Back-off restarting failed container
  Normal   Pulled                 2m (x3 over 2m45s)     kubelet, gke-jx-rocks-default-pool-119fec1e-v7p7  Container image "10.31.245.243:5000/vfarcic/go-demo-6:0.0.81" already present on machine
  Normal   Created                2m (x3 over 2m45s)     kubelet, gke-jx-rocks-default-pool-119fec1e-v7p7  Created container
  Normal   Started                2m (x3 over 2m44s)     kubelet, gke-jx-rocks-default-pool-119fec1e-v7p7  Started container
  Warning  Unhealthy              43s (x2 over 53s)      kubelet, gke-jx-rocks-default-pool-119fec1e-v7p7  Liveness probe failed: HTTP probe failed with statuscode: 404
  Warning  Unhealthy              39s (x9 over 119s)     kubelet, gke-jx-rocks-default-pool-119fec1e-v7p7  Readiness probe failed: HTTP probe failed with statuscode: 404
```

This time, liveness and readyness probes are failing. Either the application inside that Pod is not responding to the probes, or there's some other problem we did not yet discover.

Now, unless you went through the code, you cannot know that the application does not respond to requests on the root path and if we take a look at the Pod definition, we'll see that `probePath` is set to `/`. Just as with the database, Jenkins X could not know which path it should use for the probe. So, it set it to the only sensible default it could, and that's `/`.

We'll have to modify the `probePath` entry.

Please edit the *charts/go-demo-6/values.yaml* file by changing `probePath: /` entry to `probePath: /demo/hello?health=true`. Feel free to use your favorite editor for that. Make sure to save the changes once you're done, and we'll push them to GitHub.
...
Once the new build is finished, we can stop watching the activity by pressing *ctrl+c*. You'll know its done when you see the `Promoted` entry in the `Succeeded` status.

What do you think? Was our application finally up-and-running? Let's check it out.
...
To be on the safe side, we'll send a request to the application. If we are greeted from the output, we'll know that it's working as expected.
...
Here's the secret. Jenkins X does not do magic. It does some very clever things and it helps a lot by simplifying things, by installing and configuring tools, and by joining them all into a sigle easy to use process. It does a lot of heavy lifting for us and a part of that is by imporing projects into its own brace.

When we import a project Jenkins X creates a lot of things. They are not a blindless copy of files from a repository. Instead, it creates solutions tilored to our needs. The real question is how does Jenkins X know what we need? The answer is hugely related to our work. It evaluates what we did so far by scanning files in a repository where our application resides. In our example, it figured out that we are using Go, and it created a bunch of files that provide the definitions for various tools (e.g., Helm, Skaffold, Docker, etc). Yet, Jenkins X could not read our mind. It failed to figure out that we needed a MongoDB and it did not know that our applicaition requires a "special" address for Kubernetes health checks. We had to reconfigure some of the files created through the import process.

The real question is a person who imports a project has the skills to discover the cause of an issue cause by missing entries in configuration files generated during the import process. If they answer is "yes, he knows how to find out what's missing in Kubernetes definitions", "yes, he uses Helm", and many other yeses, than that person should not have a hard time figuring out things similar to those we did together. Nevertheless, Jenkins X is not made to serve only people who are Kubernets ninjas, and who know Helm, Skaffold, Docker, KNative Build, and other awesome tools Jenkins X packages. It is designed to ease the pain caused by the complexity of today's solutions. As such, it is silly to expect that everyone will go through the same process as we did. Someone often must, but the end result should not be what we just did. Instead, the solution lies in creation of build packs. We'll explore them in the next chapter. We'll try to create a solution that will allow anyone with an application written in Go and dependant on MongoDB to import it into Jenkins X and expect everything to work from the first attempt.

The application we imported is on one extreme of the spectrum. It had nothing but Go code. There was no Dockerfile, no Jenkinsfile, no Makefile, no skaffold.yaml, and certainly no Helm chart. Your applications might not be in such a state. Maybe you already created a Dockerfile, but you're missing the rest. Or, maybe you have all those except skaffold.yaml. In those cases when your project has some of the files, but not all, the import process will generate only those that are missing. It does not matter much what you do and what you don't have. Jenkins X will fill in the missing pieces.
