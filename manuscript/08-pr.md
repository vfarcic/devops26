...
W> If you're getting an error similar to `410 Issues are disabled for this repo []`, you will have to enable settings manually. Open the issues page, go to the *Features* heading, and enable *Issues* by checking the checkbox. Re-run the previous command and confirm that the issue was indeed created.
...
Please open *Jenkinsfile* in your favorite editor and add the step that follows after the line `checkout scm` of the *stage* `CI Build and push snapshot`.
