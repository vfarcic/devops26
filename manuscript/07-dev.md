...
First, Jenkins X created the `jx-edit-YOUR_USER` Namespace in your cluster. That'll be your personal piece of cluster where we'll deploy your changes to *go-demo-6*.
...
During the developement builds, we won't be passing `VERSION`, but let it be autogenerated through skaffold's "special" variable `DIGEST_HEX`. Through it, every build will be tagged using a unique string (e.g., `27ffc7f...`).
...
When we import a project, or when we create a new one using one of the quickstarts, one of the files created for us is `watch.sh`. It is a simple yet very useful script that combines the commands we run so far, and it adds a twist you can probably guess from its name. Let's take a look what's inside.
...
To be on the safe side, we assigned executable permissions to the script before we executed it. Since we used `nohup` in the second command, it'll run even if we end our session, and `&` will make sure that the watcher is running in background. Please press the enter key to go back to the terminal.
...
We saw that we can work with local files and let ksync synchronize them with DevPod. That allows us to work with our favorite IDE while ripping benefits of having a full environment somewhere else. But that's not all. We can integrate IDEs with Jenkins X.
