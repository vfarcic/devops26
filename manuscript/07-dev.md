...
To be on the safe side, we assigned executable permissions to the script before we executed it. Since we used `nohup`, it'll run even if we end out session, and `&` will make sure its running in background. Please press the enter key to go back to the terminal.
